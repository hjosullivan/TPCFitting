...
TPCFit <- function(Data,PLOT=TRUE,OverPLOT=FALSE,Model="Schoolfield",SchoolTpk=TRUE){ 
    ...
    if (length(current_dataset$OriginalTraitValue)>=5 && length(unique(current_dataset$K))>5)
        {
    ...
    #####################
          ...
          try( 
              schoolfield_nls <- nlsLM(
                  ...
                  lower=c(B0=-Inf,   E=0,    E.D=0, T_h=0),
                  upper=c(B0=Inf,    E=Inf,  E.D=Inf, T_h=273.15+150),
